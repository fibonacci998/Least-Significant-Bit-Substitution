...
//        return ( data.getData() );
    ...
    public byte[] extractBytesFromBuffer(BufferedImage buffered){
        byte[] re = null;
        try {
            // open image
    ...
    public byte[] getByteFromString(String text){
        ArrayList<Byte> t=new ArrayList();
        byte[] byteFromText=text.getBytes();
        char[] byteFromDEL=Character.toChars(127);
        t.add((byte)byteFromDEL[0]);
        for (int i=0;i<byteFromText.length;i++){
            t.add(byteFromText[i]);
        }
        t.add((byte)byteFromDEL[0]);
    ...
    private void btnHideMessageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHideMessageActionPerformed
        // TODO add your handling code here:
        //ufferedImage image=new BufferedImage(300, 300, BufferedImage.TYPE_INT_RGB);
        //Graphics2D g2=(Graphics2D) image.getGraphics();
        //canvas.paintWithSavedData(g2);
        //Container c=getContentPane();
        //c.paint(image.getGraphics());
        ...
        byte[] string=getByteFromString(txtSecretText.getText());
        
        int count=100;
        //StringBuilder binary=new StringBuilder();
        //loop all element in text string 
        for (byte b:string){
            int val=b;
            //assign 7 bit of a character in text to 7 lsb into 7 element array
            for (int i=0;i<7;i++){
                int lastDigit= ((val&128)==0?0:1);
        ...
        BufferedImage img=convertArrayToBufferedImage(array);
        stegoImage=img;
        byte[] b1=array;
        byte[] b2=extractBytesFromBuffer(img);
